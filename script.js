let snake;
let food;
let resolution = 20;// this represents the size of each block
let cols;
let rows;
let score = 0; //this is represents the score that will change as the red blocks accumilate
let gameState = 'start';
...
//Vectors create the intial blocks
}
...
function draw() {
  ...
  if (gameState === 'start') {
    ...
    snake.update();
    snake.show();
//updates the snakes position when it grows from the red blocks
    noStroke();
    fill(255, 0, 0);
    rect(food.x, food.y, 1, 1);
//this is the drawing of the food
    fill(0);
    textSize(1); 
...
//makes the snake move based on the intial x and y axis and the direction of movement it is going in
...
//Code above updates the snakes positon when its moving around or hirts itself resulting in the game to end
  endGame() {
    gameState = 'end';
  }
